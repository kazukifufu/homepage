...
class CalendarMemoScheduleApp:
    ...
    def _create_calendar_section(self):
        ...
        self.calendar_prev_button = tk.Label(
            ...
            font=("Arial", 10), width=8,  # Labelのwidthは文字数単位（Buttonより少し広めがおすすめ）
            bg=self.colors["button_bg"],
            fg=self.colors["button_fg"],
            cursor="hand2",  # カーソルを指の形に
            padx=5, pady=2,  # 内側の余白
            highlightthickness=0,  # 枠線の太さ
            highlightbackground=self.colors["button_fg"]  # 枠線の色を指定
        )
        # クリックイベントに関数をバインド
    ...
    def _display_calendar(self):
        ...
        day_colors = [self.colors["cal_fg_sun"], self.colors["cal_fg_not_sunsat"], self.colors["cal_fg_not_sunsat"], self.colors["cal_fg_not_sunsat"], self.colors["cal_fg_not_sunsat"], self.colors["cal_fg_not_sunsat"], self.colors["cal_fg_sat"]]
        ...
        for week_num, week in enumerate(month_days, start=1):
            for day_num, day in enumerate(week):
                ...
                if day != 0:
                    ...
                    # lambdaの引数 d=date_obj.date() でループ時の日付を固定します
                    ...
                    if date_obj.date() == self.selected_date:
                        selected_bg = self.colors["cal_selected"]  # 選択時の背景色（薄い水色など）
                        # フレームと、その中にあるラベルすべての背景色を更新
    ...
    def _select_date(self, date, holiday_name):
        ...
        # スケジュールエリアの日付表示を更新
        self.schedule_date_label.config(text=f"{self.selected_date.year}年{self.selected_date.month}月{self.selected_date.day}日")
    ...
    def _create_memo_section(self):
        ...
        self.note_date_label.config(text=f"{self.selected_date.year}年{self.selected_date.month}月{self.selected_date.day}日")
    ...
    def _create_schedule_section(self):
        ...
        self.schedule_daily_button = tk.Label(
            ...
            font=("Arial", 10), width=12,  # Labelのwidthは文字数単位（Buttonより少し広めがおすすめ）
            bg=self.colors["button_bg"],
            fg=self.colors["button_fg"],
            cursor="hand2",  # カーソルを指の形に
            padx=5, pady=2,  # 内側の余白
        ...
        if self.schedule_view_mode == "daily":
            ...
            for cat, clr, txt, col in configs:
                ...
                add_label = tk.Label(
                    ...
                    font=("Arial", 10), width=15,
                    bg=clr,
                    fg=self.colors.get("button_fg", "white"),  # 文字色。定義がなければ白
                    cursor="hand2",
                    padx=5, pady=2,
            ...
            tk.Label(self.schedule_header_lower_frame, text="※週次モード：予定のみを表示します", fg=self.colors["button_fg"], bg=self.colors["button_bg"]).pack()
    ...
    def open_schedule_input_dialog(self, category, edit_id=None, initial_data=None):
        ...
        def _save_schedule():
            start_value, end_value, name_value = task_start_entered.get(), task_end_entered.get(), task_name_entered.get()
            try:
        ...
        tk.Button(schedule_input_dialog, text="保存", command=_save_schedule, bg=self.colors["button_bg"], width=15).pack(pady=20)
    ...
    def _save_schedule_task(self, cat, eid, vals):
        ...
        with sqlite3.connect(DB_PATH) as conn:
            if eid:
                conn.execute('UPDATE events SET start_time=?, end_time=?, task_name=? WHERE id=?', (s, e, t, eid))
            else:
                conn.execute('INSERT INTO events (event_date, category, start_time, end_time, task_name) VALUES (?,?,?,?,?)',
                             (d_str, cat, s, e, t))
        self._load_schedule_data()
    ...
    def _create_schedule_canvas(self, parent):
        ...
        # 1日の総高さ = 上下の余白(20*2) + (24時間 * 1時間の高さ)
        total_height = 20 + (END_HOUR - START_HOUR) * HOUR_HEIGHT + 20

        # スクロール範囲を (x1, y1, x2, y2) で指定
        # これにより、指定した高さ（24:00の少し下）でスクロールが止まります
        ...
        self.schedule_canvas.bind("<MouseWheel>", lambda e: self.schedule_canvas.yview_scroll(int("-1" if e.delta > 0 else "1"), "units"))
        ...
        with sqlite3.connect(DB_PATH) as conn:
            if self.schedule_view_mode == "daily":
                ...
                for row in cur.fetchall():
                    event_date = row[0]
                    self._schedule_cache.setdefault(event_date, []).append(row[1:])
    ...
    def _draw_schedule(self, canv):
        ...
        if self.schedule_view_mode == "daily":
            ...
            # 選択日の週の日曜日を算出 (weekday(): 月=0 ... 日=6)
            offset = (self.selected_date.weekday() + 1) % 7
            start_date = self.selected_date - timedelta(days=offset)
            end_date = start_date + timedelta(days=6)
            self.schedule_date_label.config(text=f"{start_date.strftime('%Y/%m/%d')} 〜 {end_date.strftime('%m/%d')} (週間)")
        ...
        for h in range(START_HOUR, END_HOUR + 1):
            ...
            canv.create_text(TIME_AXIS_WIDTH - 10, y_pixel, text=f"{h:02d}:00", anchor="e", font=("Arial", 9), fill=self.colors["frame_fg"])
            canv.create_line(TIME_AXIS_WIDTH, y_pixel, width - 20, y_pixel, fill=self.colors["frame_fg"])

        # 2. 列の見出しと境界線
        for i in range(num_cols):
            x_left = TIME_AXIS_WIDTH + (i * col_w)
            canv.create_text(x_left + col_w/2, 10, text=col_labels[i], font=("Arial", 8, "bold"), fill=self.colors["frame_fg"])
            if i > 0:
        ...
        if self.schedule_view_mode == "daily":
            ...
            for i in range(7):
                ...
                for s_time, e_time, t_name in self._schedule_cache.get(curr, []):
                    y_s = self._convert_time_to_y_axis(s_time)
                    y_e = self._convert_time_to_y_axis(e_time)
                    self._draw_task_bar(canv, base_x, y_s, y_e, t_name, self.colors["schedule_plan"], col_w)
    ...
    def _draw_task_bar(self, canv, base_x, y_s, y_e, task_name, clr, w):
        ...
        # 文字がはみ出さないよう、幅が狭い場合は省略 (Weeklyモード向け)
        ...
        canv.create_text(text_x, text_y, text=display_text, anchor="w", font=("Arial", 9, "bold"), fill=clr)
    ...
    def show_registered_task(self, category):
        ...
        def refresh_tree():
            ...
            with sqlite3.connect(DB_PATH) as conn:
                cur = conn.execute('SELECT id, start_time, end_time, task_name FROM events WHERE category=? AND event_date=? ORDER BY start_time ASC', (category, str(self.selected_date)))
        ...
        btn_f.pack()
        tk.Button(btn_f, text="修正", command=lambda: self._on_edit(tree, category, refresh_tree)).pack(side=tk.LEFT)
        tk.Button(btn_f, text="削除", command=lambda: self._on_del(tree, refresh_tree)).pack(side=tk.LEFT)

    def _on_edit(self, tree, category, refresh_func):
        """ツリーからの編集。"""
        sel = tree.selection()
        if not sel:
            return
        it = tree.item(sel[0], "values")
        self.open_schedule_input_dialog(category, edit_id=it[0], initial_data={
            's': it[1], 'e': it[2], 't': it[3], 'callback': refresh_func
        })
