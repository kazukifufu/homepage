...
class CalendarView:
    ...
    def bind_prev(self, callback: Callable[[], None]) -> None:
        """「前月」ボタンクリック時のコールバックを登録する。"""
        self._prev_btn.bind("<Button-1>", lambda e: callback())

    def bind_next(self, callback: Callable[[], None]) -> None:
    ...
    def render(
        ...
        month: int,
        selected_date: date,
        on_date_click: Callable[[date, str], None],
    ) -> None:
        """カレンダーグリッドを再描画する。
        ...
        Args:
            year: 表示年。
            month: 表示月。
            selected_date: 選択中の日付（ハイライト用）。
            on_date_click: 日付セルクリック時のコールバック。
        """
        ...
        self._render_day_cells(year, month, month_days, selected_date, on_date_click)
        self._configure_grid(month_days)

    def _render_weekday_header(self) -> None:
        """曜日ヘッダー行を描画する。"""
    ...
    def _render_day_cells(
        ...
        for week_idx, week in enumerate(month_days, start=1):
            for day_idx, day in enumerate(week):
                ...
                date_obj = datetime(year, month, day)  # ← ここで生成して渡す
                self._render_day_cell(
                    cell, date_obj, day_idx, selected_date, on_date_click
                )

    def _render_day_cell(
        self,
        cell: tk.Frame,
        date_obj: datetime,         # ← year/month/day の3引数を1つに集約
        day_idx: int,
        selected_date: date,
        on_date_click: Callable[[date, str], None],
    ) -> None:
    ...
    def _configure_grid(self, month_days: list[list[int]]) -> None:
        """グリッドの伸縮設定を行う。"""
        for i in range(7):
